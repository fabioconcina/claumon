...
bugs unless this document is updated.
...
\section{Forecast distribution at reset}
...
\[
  \sigma_F^2 = \underbrace{\Dtrem^2\,\taupostsq}_{\text{rate uncertainty}}
            + \underbrace{\Dtrem\,\sigsessionsq}_{\text{path noise}}.
\]
Both contributions are Gaussian under the model, so $u(\Treset) \sim
\mathcal{N}(F, \sigma_F^2)$. The two terms have different scaling: rate
uncertainty is quadratic in horizon (dominates at long horizons), path noise
is linear (dominates at short).

\paragraph{80\% CI.} $[F - z_{0.9}\,\sigma_F,\ F + z_{0.9}\,\sigma_F]$,
clipped to $[0, 1]$ for display. The unclipped $F$ is retained for ETA
computation.
...
\paragraph{Why $\hat b$ is discarded.} Conceptually $\hat b$ estimates the
same quantity as $\taupostsq$ in \S\ref{sec:rate} (the rate-estimation
variance), just as a historical average instead of a fresh per-forecast
value. The per-forecast version in \S\ref{sec:rate} uses today's actual
snapshots, so it is strictly more informative; $\hat b$ is redundant.

The $b\,\delta_f^2$ term still has to be \emph{present in the regression},
though. Without it, the $\delta_f^2\,\bar\tau^2$ piece of $e_f^2$ has
nowhere to go but $\hat a$, biasing it upward. So $b$ is included as a
nuisance parameter (analogous to adjusting for a confounder in a
regression: we control for its contribution without caring about its fitted
value), and discarded once $\hat a$ is clean.
...
Calibration: $\sigsessionsq = 2.5 \times 10^{-3}\,\mathrm{h}^{-1}$.
...
\]
\[
  \sigma_F^2 \approx 9 \cdot 6.19 \times 10^{-5}
    + 3 \cdot 2.5 \times 10^{-3}
    \approx 5.6 \times 10^{-4} + 7.5 \times 10^{-3}
...
80\% CI: $[0.490 - 1.282 \cdot 0.090,\ 0.490 + 1.282 \cdot 0.090]
\approx [0.375,\ 0.605]$. Display: ``Projected 49\% by reset (80\% CI:
38\%--60\%)''.

Path noise dominates rate uncertainty by $13\times$. That is: we know the
current slope well; the spread is ``you might pivot.''
